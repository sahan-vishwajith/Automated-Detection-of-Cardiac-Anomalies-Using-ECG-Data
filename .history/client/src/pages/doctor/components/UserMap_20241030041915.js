...
const generateColorByDensity = (density, maxDensity) => {
  // Check if density is undefined
  if (density === undefined) {
    // Return a blank color (transparent)
    return 'rgba(0, 0, 0, 0)'; // You can change this to any color you prefer
  }
  ...
  // Define color ranges using if statements
  if (normalizedDensity < 0.2) {
    // Very low density
    r = 0; // Red is low
    g = Math.floor(255 * (normalizedDensity / 0.2)); // Gradually increase green
    b = 0; // Blue stays low
    ...
    // Medium density
    r = Math.floor(255 * ((normalizedDensity - 0.4) / 0.2)); // Increase red
    g = 255; // Green stays high
    b = 0; // Blue stays low
  } else if (normalizedDensity < 0.8) {
    // Medium high density
    r = 255; // Red is high
    g = Math.floor(255 * (1 - ((normalizedDensity - 0.6) / 0.2))); // Decrease green
    b = 0; // Blue stays low
  } else {
    // High density
    r = 255; // Red is high
  ...
  console.log(`Density: ${density}, Color: rgb(${r}, ${g}, ${b})`);

  return `rgb(${r}, ${g}, ${b})`;
};

// Example usage
const maxDensity = 40; // Example maximum density
const densities = [undefined, 0, 5, 10, 15, 20, 25, 30, 35, 40];

densities.forEach(density => {
  console.log(`Density: ${density}, Color: ${generateColorByDensity(density, maxDensity)}`);
});
    ...
    <div style={styles.mapContainer}>
      <MapContainer center={[8.0, 81.0]} zoom={7} style={{ height: '100%', width: '100%' }}>
        ...
        <GeoJSON 
          data={geoJsonData} 
          style={(feature) => ({
            color: getColor(feature,locationCounts.locationCounts), // Set border color based on feature name
            weight: 1,
            fillOpacity: 0.3,
            fillColor: getColor(feature,locationCounts.locationCounts), // Set fill color based on feature name
          })} 
        />
