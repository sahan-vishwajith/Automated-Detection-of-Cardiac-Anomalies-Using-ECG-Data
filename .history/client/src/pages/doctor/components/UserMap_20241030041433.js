...
const generateColorByDensity = (density, maxDensity) => {
  ...
  if (normalizedDensity < 0.2) {
    ...
    b = 255; // Blue stays high
  } else if (normalizedDensity < 0.4) {
    // Medium low density
    r = Math.floor(255 * ((normalizedDensity - 0.2) / 0.2)); // Increase red
    g = 255; // Green stays high
    b = 255 - Math.floor(255 * ((normalizedDensity - 0.2) / 0.2)); // Decrease blue
  } else if (normalizedDensity < 0.6) {
    // Medium density
    r = 255; // Red stays high
    g = Math.floor(255 * (1 - ((normalizedDensity - 0.4) / 0.2))); // Decrease green
    b = 0; // Blue stays low
  } else if (normalizedDensity < 0.8) {
    // Medium high density
    r = 255 - Math.floor(255 * ((normalizedDensity - 0.6) / 0.2)); // Decrease red
    g = 0; // Green stays low
    b = Math.floor(255 * ((normalizedDensity - 0.6) / 0.2)); // Increase blue
  } else {
    // High density
    r = 0; // Red stays low
    g = 0; // Green stays low
