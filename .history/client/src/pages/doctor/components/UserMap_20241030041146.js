...
const generateColorByDensity = (density, maxDensity) => {
  ...
  // Exponential scaling for more impact on color change
  const scaleFactor = 2; // Adjust this factor to increase/decrease sensitivity
  const adjustedDensity = Math.pow(normalizedDensity, scaleFactor);

  // Define color ranges (from light to dark)
  const r = Math.floor(255 * adjustedDensity); // Red increases with density
  const g = Math.floor(255 * (1 - adjustedDensity)); // Green decreases with density
